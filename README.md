...
- **空白忽略**：支持开启 `repeat_ignore_whitespace`，忽略文本前后的空格和换行比对，防止微小格式差异绕过拦截。
...
- `~输出过滤配置`：查看当前拦截器的各项运行状态和配置项概要。
- `~开启重复拦截` / `~关闭重复拦截`：一键控制重复检测的生命周期。
- `~设置重复拦截提示 <内容>`：快速设置重复过滤被挡住时的气泡反馈，不填内容则还原为静默拦截。
